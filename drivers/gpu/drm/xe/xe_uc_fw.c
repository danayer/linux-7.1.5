...
static int parse_css_header(struct xe_uc_fw *uc_fw, const void *fw_data, size_t fw_size)
{
	...
	if (unlikely(size != sizeof(struct uc_css_header))) {
		if (uc_fw->type == XE_UC_FW_TYPE_HUC && size == 629248) {
			printk(KERN_INFO "DG2 DEBUG: Allowing GSC manifest size (629248) for HuC probe\n");
			release->major = 7;
			release->minor = 10;
			return 0;
		}
...
{
	struct xe_gt *gt = uc_fw_to_gt(uc_fw);
	const struct xe_gsc_manifest_header *manifest = data;
	struct xe_uc_fw_version *release = &uc_fw->versions.found[XE_UC_FW_VER_RELEASE];

	if (size < sizeof(*manifest)) {
		xe_gt_err(gt, "Direct manifest too small! %zu < %zu\n", size, sizeof(*manifest));
		return -ENODATA;
	}
...
static int parse_headers(struct xe_uc_fw *uc_fw, const struct firmware *fw)
{
	...
	/*
	 * Short-circuit for direct GSC manifests (signature 0x4).
	 * This prevents falling through to legacy CSS parsers which would
	 * fail with -EPROTO due to size mismatch.
	 */
	if (fw->size >= sizeof(u32) && *(const u32 *)fw->data == 0x4) {
		printk(KERN_INFO "DG2 DEBUG: Direct GSC manifest detected, bypassing legacy parsers\n");
...
// ...existing code...
	...
	case XE_UC_FW_TYPE_HUC:
		/*
		 * DG2 HuC firmware is wrapped in a BPDT -> CPD -> CSS structure.
		 * We only attempt GSC parsing on DG2 platforms to avoid crashing
		 * standard integrated GPUs (e.g. TGL).
		 */
		if (uc_fw_to_xe(uc_fw)->info.platform == XE_DG2) {
			ret = parse_gsc_layout(uc_fw, fw->data, fw->size);
			if (ret)
				return ret;
		} else {
			/* Legacy CSS initialization for standard GPUs like Tiger Lake */
			ret = parse_css_header(uc_fw, fw->data, fw->size);
			if (ret == 0)
				return ret;
		}
		return ret;
