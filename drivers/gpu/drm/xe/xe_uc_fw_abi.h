...
/* GSC Binary Structures */

struct xe_gsc_version {
	u16 major;
	u16 minor;
	u16 hotfix;
	u16 build;
...
struct xe_gsc_partition {
	u32 offset;
	u32 size;
} __packed;

struct xe_gsc_layout_pointers {
	...
	struct xe_gsc_partition datap;
	struct xe_gsc_partition boot1;
	struct xe_gsc_partition boot2;
	struct xe_gsc_partition boot3;
	struct xe_gsc_partition boot4;
	struct xe_gsc_partition boot5;
	struct xe_gsc_partition temp_pages;
} __packed;

struct xe_gsc_bpdt_header {
	u32 signature;
	...
	u32 crc32;
	u32 build_version;
	struct xe_gsc_version tool_version;
} __packed;

#define XE_GSC_BPDT_HEADER_SIGNATURE 0x000055AA

struct xe_gsc_bpdt_entry {
	u32 type;
	u32 sub_partition_offset;
	u32 sub_partition_size;
} __packed;

#define XE_GSC_BPDT_ENTRY_TYPE_MASK GENMASK(15, 0)
#define XE_GSC_BPDT_ENTRY_TYPE_GSC_RBE 0x1

struct xe_gsc_cpd_header_v2 {
	u32 header_marker;
	u32 num_of_entries;
	u8 header_version;
	u8 entry_version;
...
#define XE_GSC_CPD_HEADER_MARKER 0x44504324

struct xe_gsc_cpd_entry {
	u8 name[12];
	u32 offset;
	u32 length;
	u8 reserved[4];
...
#define XE_GSC_CPD_ENTRY_OFFSET_MASK GENMASK(24, 0)
#define XE_GSC_CPD_ENTRY_HUFFMAN_COMP BIT(25)

struct xe_gsc_manifest_header {
	u32 header_type;
	u32 header_length;
	u32 header_version;
	u32 flags;
	...
	u32 header_id;
	u32 internal_data;
	struct xe_gsc_version fw_version;
	u32 security_version;
	struct xe_gsc_version meu_kit_version;
