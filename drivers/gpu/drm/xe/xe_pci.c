 ...
 */
static int xe_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	struct xe_device *xe;
	const struct xe_device_desc *desc = (const struct xe_device_desc *)id->driver_data;
	const struct xe_subplatform_desc *subplatform_desc;
	int ret;
	...
	ret = xe_info_init_early(xe, desc, subplatform_desc);
	if (ret)
		return ret;

	xe_pci_rebar_resize(xe);

	ret = xe_device_probe_early(xe);
	/*
	...
	ret = xe_info_init(xe, desc);
	if (ret)
		return ret;

	ret = xe_display_probe(xe);
	if (ret)
		return ret;
